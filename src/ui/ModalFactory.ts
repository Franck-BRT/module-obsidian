...
class DuplicateSubtasksModal extends Modal {
  ...
  onOpen(): void {
    ...
    contentEl.createEl('p', {
      text: t('project.duplicateConfirm', { title: this.taskTitle }),
      cls: 'pm-confirm-text'
    })
...
export function openProjectCreate(plugin: PMPlugin, program = false, parentPath = '', template = false): void {
  new ProjectCreateModal(plugin.app, plugin, program, template, parentPath).open()
}
