...
export class ProjectStore implements TaskSource {
      ...
      if (file instanceof TFile) {
        ...
        await this.app.vault.process(file, (content) => {
          ...
          return serializeProject(
            ...
            // Anything the user typed into the note themselves; regenerating the
            // body without it would drop it silently on every save.
            projectBodyRemainder(body, project.icon, project.title, project.description)
          )
        })
        this.hydratedBodies.add(project)
      } else {
  ...
  private async spawnNextOccurrence(project: Project, taskId: string): Promise<void> {
    ...
    try {
      ...
      console.error(`[Black Projects] Failed to create the next occurrence of "${task.title}":`, e)
  ...
  private reportCycles(cycleIds: Set<string>, loaded: Map<string, Project>): void {
    ...
    console.warn('[Black Projects] Dependency cycle, these tasks are not scheduled:', [...cycleIds])
