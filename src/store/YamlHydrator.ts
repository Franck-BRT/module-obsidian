...
import { parseTime } from './Clock'
import type { ImpactLevel } from './ZoneImpact'
import { PROGRAM_FRONTMATTER_KEY, stripAutoGeneratedContent, TEMPLATE_FRONTMATTER_KEY } from './YamlParser'
...
/** A level a note declares, or nothing — which means the project's, then the default. */
function readImpactLevel(raw: unknown): ImpactLevel | undefined {
  return raw === 'blocking' || raw === 'caution' || raw === 'info' ? raw : undefined
}

/** A time of day from a note, or nothing when what is there is not one. */
function readTime(raw: unknown): string | undefined {
...
export function mapRawToTask(r: Record<string, unknown>, overrides?: Partial<Task>): Task {
  return makeTask({
    ...
    // Forgiving, because a note is edited by hand: `9h30` typed into the frontmatter is
    // someone saying half past nine, and refusing it would silently drop what they wrote.
    ...
    impactLevel: readImpactLevel(r.impactLevel),
  ...
  return {
    ...
    impactRole:
      frontmatter.impactRole === 'emitter' || frontmatter.impactRole === 'receiver'
        ...
        : undefined,
    impactLevel: readImpactLevel(frontmatter.impactLevel),
    ...
    ...(frontmatter[TEMPLATE_FRONTMATTER_KEY] === true ? { template: true } : {}),
    taskIndex: new Map()
  }
}
